...
class PathfindingLambdaTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        # ใช้ข้อมูลจริงจาก Database/graph.json เพื่อให้ test ใกล้เคียงการใช้งานจริง
    ...
    def assert_valid_real_graph_path(self, path, start_node, end_node):
        # ตรวจว่า path เริ่มและจบถูก node และทุกช่วงทางมี edge เชื่อมกันจริงใน graph
    ...
    def test_find_shortest_path_between_real_first_floor_rooms(self):
        # เคสหาเส้นทางระหว่างห้องจริงบนชั้น 1
    ...
    def test_find_shortest_path_between_nearby_real_rooms(self):
        # เคสห้องใกล้กัน เพื่อเช็กว่า algorithm คืน path สั้นและต่อเนื่อง
    ...
    def test_find_shortest_path_between_real_floors(self):
        # เคสข้ามชั้น โดยใช้ node บันไดที่มี edge เชื่อมระหว่างชั้นจริง
    ...
    def test_lambda_handler_loads_database_graph_file(self):
        # ไม่ส่ง graph เข้า event เพื่อยืนยันว่า lambda_handler โหลด Database/graph.json เอง
    ...
    def test_lambda_handler_rejects_missing_real_node(self):
        # เคส node ปลายทางไม่มีใน graph ต้องตอบ 404 พร้อม error
